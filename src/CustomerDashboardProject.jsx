...
const navItems = [
  ...
  { key: "invoices", label: "STAPLES INVOICES", icon: ReceiptText },
...
function Shell({ page, setPage, children, showFooterTabs = false }) {
  return (
    <div className="min-h-screen bg-[#eef2f6] text-[12px] text-black" style={{ fontFamily: "Aptos, Arial, sans-serif" }}>
      <div className="fixed left-0 right-0 top-0 z-20 h-6 bg-[#5d5a5a] px-7 text-[11px] font-bold leading-6 text-white">Last Refreshed: 04/06/2026 5:00 AM EST</div>
      <aside className="fixed left-0 top-6 z-10 h-[calc(100vh-24px)] w-[214px] border-r border-gray-200 bg-white shadow-md">
        <div className="flex h-[50px] items-center gap-2 border-b border-gray-100 px-4">
          <span className="text-2xl font-bold text-[#e1261c]">▱ Staples.</span>
      ...
      {showFooterTabs && (
        <div className="fixed bottom-0 left-0 right-0 z-30 ml-[0px] flex h-[39px] bg-white text-xs shadow-[0_-1px_3px_rgba(0,0,0,0.15)]">
          {["Purchase Orders", "Staples Invoices", "Inventory", "Accounts Payable", "Asset Lookup", "Shipment Tracking", "Report Library"].map((t) => (
    ...
    <div className="grid grid-cols-2 gap-3 pb-10">
      ...
      <Card className="p-4">
        ...
        <div className="mt-6 grid grid-cols-2 gap-8 px-6">
          <div className="flex items-center gap-4"><User size={48} fill={RED} color={RED} /><div><b>Dedicated Account Manager</b><br />John Doe<br /><a className="text-blue-700 underline">John.Doe@Staples.com</a><br />(800) 123-4567</div></div>
          <div className="flex items-center gap-4"><Users size={50} fill={RED} color={RED} /><div><b>Strategic Support Team</b><br />SupportTeam@Staples.com<br />Monday - Friday<br />8AM - 5PM EST</div></div>
        </div>
      </Card>
      <Card className="p-4">
...
export default function CustomerDashboardProject() {
  ...
  const content = useMemo(() => {
    ...
    if (page === "invoices") return <TablePage title="Your Invoice History" filters={["Staples Invoice","PO Number","Timeframe","Ship-to Contact","Ship-to Address","Ship-to City","Ship-to State"]} columns={["INVOICE NUMBER","PO NUMBER","INVOICE DATE","CUSTOMER NAME","SHIP-TO NAME","SHIP-TO LOCATION","AMOUNT"]} rows={invoices} button="View Invoice Details"/>;
    if (page === "inventory") return <TablePage title="Your Inventory" filters={["Staples SKU","Manufacturer Part Number","SKU Description","Brand","Product Category"]} columns={["SKU","DESCRIPTION","PRODUCT CATEGORY","LINE OF BUSINESS","AVAILABLE UNITS","INCOMING UNITS","UNITS PURCHASED WEEKLY","STOCK COVERAGE (WEEKS)","LAST STOCKED DATE","DAYS AGED","90 DAY USAGE","LARGEST DEVIATION"]} rows={inventory.map(r=>r.map((c,i)=> i===7 ? <><Dot color={Number(c)<4?AMBER:GREEN}/> {c}</> : i===9 && Number(c)>90 ? <span className="bg-yellow-200 px-5 py-2">{c}</span> : c))} button="View SKU Details"/>;
    if (page === "inventoryDetail") return <InventoryDetail/>;
    if (page === "ap") return <TablePage title="Your Accounts Payable" filters={["Transaction Type","PO Number","Staples Invoice","Invoice Status","Aging Category"]} summary={[{label:"TOTAL OUTSTANDING:",value:"$524,572",pct:0},{label:"CURRENT INVOICES",value:0,pct:0},{label:"DUE THIS WEEK",value:7,pct:22},{label:"DUE NEXT WEEK",value:0,pct:0},{label:"PAST DUE",value:26,pct:88}]} columns={["PO NUMBER","STAPLES INVOICE","INVOICE DATE","STATUS","DUE DATE","AMOUNT","APPLIED","OPEN"]} rows={invoices.map(r=>[r[1],r[0],r[2], r[2].includes("2025")||r[2].includes("2/")||r[2].includes("1/")?"Past Due":"Due This Week","4/3/2026",money(r[6]),"$0.00",money(r[6])])} type="ap"/>;
    if (page === "assets") return <TablePage title={<span>Your Hardware Assets <Info size={18} className="inline text-blue-500"/></span>} filters={["Device Serial Number","SKU","Manufacturer Part Number","SKU Description","PO Number","Staples Invoice","Status"]} columns={["SERIAL NUMBER","SKU","MANUFACTURER PART NUMBER","DESCRIPTION","PO NUMBER","STAPLES INVOICE","ASSET AGE (YEARS)","STATUS"]} rows={assets} type="asset"/>;
    if (page === "shipments") return <TablePage title="Your Shipment Tracking" filters={["Staples Invoice","PO Number","Timeframe","Carrier","Contents"]} columns={["INVOICE NUMBER","PO NUMBER","SHIP DATE","TRACKING NUMBER","CARRIER","SHIP TO LOCATION","CONTENTS"]} rows={shipments.map(r=>r.map((c,i)=> i===3 ? <a className="text-blue-600 underline">{c}</a> : c))}/>;
